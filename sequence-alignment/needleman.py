...
class Needleman:
    ...
    def _set_gap_row_column(self):
        ...
        """
        for i in range(self.sequence1_length+1):
            self.matrix[i][0] = i * self.scores["gap"]
        for j in range(self.sequence2_length+1):
            self.matrix[0][j] = j * self.scores["gap"]
    ...
    def _populate_matrix(self):
        ...
        for i in range(1, self.sequence1_length+1):
            for j in range(1, self.sequence2_length+1):
                ...
                self.matrix[i][j] = max(diagonal_score, horizontal_score, vertical_score)
        if DEBUG:
            self._print_matrix()
    ...
    def _traceback(self) -> Tuple[str, str, int]:
        ...
        if DEBUG:
            ...
            print("Traceback")
            print(LINE)
        i, j = self.sequence1_length, self.sequence2_length
        while i > 0 or j > 0:
            if DEBUG:
            ...
            if i > 0 and j > 0 and self.matrix[i][j] == diagonal_score:
                aligned_sequence1 = self.sequence1[i-1] + aligned_sequence1
                aligned_sequence2 = self.sequence2[j-1] + aligned_sequence2
                i -= 1
                j -= 1
            # vertical
            elif i > 0 and self.matrix[i-1][j] + self.scores["gap"] == self.matrix[i][j]:
                aligned_sequence1 = self.sequence1[i-1] + aligned_sequence1
        ...
        alignment_score = self._find_alignment_score(aligned_sequence1, aligned_sequence2)
...
def main():
    ...
    # Solve
    needleman = Needleman(S1, S2, match, mismatch, gap)
    aligned_sequence1, aligned_sequence2, score = needleman.find_optimal_alignment()

    # Output
    print(LINE)
    print(f"Alignment (Score: {score})")
    print(LINE)
    print(f"Sequence 1: {aligned_sequence1}")
    print(f"Sequence 2: {aligned_sequence2}")
    print(LINE)
